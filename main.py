...
# check packet
def process_packet(packet, policies):
    try:
        # using Scapy
        if IP in packet:
            ip_header = packet[IP]  
            ...
            dest_domain = resolve_ip_to_domain(dest_ip)

            packet_info = {
                'ip': dest_ip,
                'domain': dest_domain,
                'protocol': 'TCP' if TCP in packet else 'UDP',  
                'port': str(packet[TCP].dport if TCP in packet else packet[UDP].dport)
            }
            #using loop to match with policies rules
            for rule in policies:
                if rule['ip'] == packet_info['ip'] and rule['domain'] == packet_info['domain']:
                    logging.info(f"Blocked packet: {packet_info}")
                    return False  
        ...
        return True  # Accept the packet if there’s an error  
